...
An example workload can import `service/s3` and call `Client.PutObject` normally. The generator resolves the direct import through `go.mod`, reads the package manifest, loads the embedded extension definition, and emits an `aws.object_store` Condition without any explicit no-op declaration in the application source.
